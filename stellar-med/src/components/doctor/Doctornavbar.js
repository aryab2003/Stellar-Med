...
export default function Doctornavbar(props) {
  const navigate = useNavigate();
  const [patientURL, setPatientURL] = useState(
    `/Patientlist?email=${props.data}`
  );

  useEffect(() => {
    // Update the patientURL when props.data changes
    setPatientURL(`/Patientlist?email=${props.data}`);
  }, [props.data]);
  const handleSignout = () => {
    signOut(auth)
      .then(() => {
                  ...
                  <div className="hidden md:block">
                    <div className="ml-10 flex items-baseline space-x-4">
                      {navigation.map((item) => (
                        <a
                          key={item.name}
                          href={item.name === 'Patients' ? patientURL : item.href}
